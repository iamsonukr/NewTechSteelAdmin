...
export default function ResetPasswordForm() {
  ...
  const {login,logout,profileData }=useAuth()
  ...
  const signIn = async (e) => {
    ...
    try {
      const response = await adminService.post('/user/login', {
      ...
      if (response.data.status === "success") {
        ...
        // Use AuthContext login function to store token and role
      ...
      // Clear auth context on login failure
      logout();
      ...
      if (error.response?.status === 401) {
        ...
        setError("Please check your phoneNumber format");
        ...
        <div>
          <div className="mb-5 sm:mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Enter your registered emailt to get password reset instructions.
            </p>
          </div>
          <div>
