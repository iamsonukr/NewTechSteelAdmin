...
import { getProductCategoryById, updateProductCategory } from "../../api/services/productCategory.service";
...
export default function ProductCategoryEdit() {
  ...
  useEffect(() => {
    getProductCategoryById(id)
      .then(res => {
        ...
        if (c.image) setExistingImage(`${import.meta.env.VITE_BACKEND_URL?.replace('/api','')}${c.image}`);
      })
  ...
  return (
    <div>
      <PageHeader title="Edit Product Category" breadcrumbs={[{ label: "Dashboard", to: "/dashboard" }, { label: "Product Categories", to: "/product-categories" }, { label: "Edit" }]} />
      <div className="max-w-2xl bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="text" value={formData.name} onChange={(e) => setFormData(p => ({ ...p, name: e.target.value }))} className="h-11 w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:text-white/90 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/20 dark:bg-gray-900" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Description</label>
            <textarea value={formData.description} onChange={(e) => setFormData(p => ({ ...p, description: e.target.value }))} rows={3} className="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:text-white/90 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/20 dark:bg-gray-900 resize-none" />
          </div>
          <ImageUpload label="Category Image" value={existingImage} onChange={setImage} />
