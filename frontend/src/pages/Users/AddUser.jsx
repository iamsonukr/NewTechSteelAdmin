...
const AddUser = ({ isOpen, closeModal, user, fetchUsers }) => { 
    const BaseImageUrl=import.meta.env.VITE_BACKEND_UPLOAD_URL
        ...
        if (isOpen) {
            if (user) {
                ...
                if (user.profileImage) {
                    setProfileImagePreview(BaseImageUrl+user.profileImage);
                }

                // Set existing frame previews if available
                setFramePreviews({
                    frameOne: user.frameOne || null,
                    frameTwo: user.frameTwo || null,
                    frameThree: user.frameThree || null
                });
            } else {
    ...
    const handleFrameChange = (frameName) => (e) => {
        ...
        if (validation.isValid) {
            ...
            // Create preview URL for the frame
            const previewUrl = URL.createObjectURL(file);
            setFramePreviews(prev => ({
                ...prev,
                [frameName]: previewUrl
            }));
    ...
    const validateForm = () => {
        ...
        // Bio validation - Updated to 1000 characters
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true);
            const response = await adminService.delete(`/user/delete-user/${user._id}`);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // FIXED: Always append bio, even if empty, to ensure updates work
            ...
            if (user) {
                ...
                response = await adminService.put(`/user/admin/update-profile/${user._id}`, payload);
                ...
                // Create new user
                response = await adminService.post('/user/register', payload, {
                    headers: {
                        ...
                        <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
                            <div className="mt-7">
                                ...
                                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                                    ...
                                    {/* Bio Field - Updated to 1000 characters */}
                                    <div className="col-span-2">
                                        <Label htmlFor="bio">
                                            Bio <span className="text-gray-400 text-xs">(max 1000 characters)</span>
                                            <span className="text-gray-400 text-xs"> (Optional)</span>
                                        </Label>
                                        <textarea
                                            id="bio"
                                    ...
                                    <div className="col-span-2 lg:col-span-1">
                                        ...
                                        {/* Frame One Preview */}
                                        {framePreviews.frameOne && (
                                            <div className="mt-3">
                                                <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">Preview:</p>
                                                <img 
                                                    src={BaseImageUrl+framePreviews.frameOne} 
                                                    alt="Frame One preview" 
                                                    className="w-32 h-32 object-contain border-2 border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800"
                                                />
                                            </div>
                                        )}
                                    </div>

                                    {/* Frame Two with Preview */}
                                    <div className="col-span-2 lg:col-span-1">
                                        ...
                                        {/* Frame Two Preview */}
                                        {framePreviews.frameTwo && (
                                            <div className="mt-3">
                                                <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">Preview:</p>
                                                <img 
                                                    src={BaseImageUrl+framePreviews.frameTwo} 
                                                    alt="Frame Two preview" 
                                                    className="w-32 h-32 object-contain border-2 border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800"
                                                />
                                            </div>
                                        )}
                                    ...
                                    <div className="col-span-2 lg:col-span-1">
                                        ...
                                        {/* Frame Three Preview */}
                                        {framePreviews.frameThree && (
                                            <div className="mt-3">
                                                <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">Preview:</p>
                                                <img 
                                                    src={BaseImageUrl+framePreviews.frameThree} 
                                                    alt="Frame Three preview" 
                                                    className="w-32 h-32 object-contain border-2 border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800"
                                                />
                                            </div>
                                        )}
